...
public class ModifyProductController implements Initializable {
    ...
    public static Product selectedProduct; // = new Product(); maybe?
    
    /**
     Called to initialize the Modify Product controller class after its root element has been completely processed. 
     TableView Columns are set up and the partsSearch TextField search functionality is implemented.
    ...
    /**
     This method listens for the Add button on the Modify Product screen being pressed. The Add button will add a Part from the total list of available Parts to the associated Parts list
     for the Product to be modified on this screen. If the user has not made a selection, an error is generated indicating this to the user.
    ...
    public void addAssociatedPartButtonPushed(ActionEvent event) throws IOException {
        ...
        if (partsTableView.getSelectionModel().isEmpty()) {
            ...
            if (selectedProduct.getAllAssociatedParts().contains(associatedPart)) {
    ...
    public void removeAssociatedPartButtonPushed(ActionEvent event) throws IOException {
        ...
        if (associatedPartsTableView.getSelectionModel().isEmpty()) {
            ...
            //if they click Remove, then .deleteAssociatedPart(selectedAssociatedPart);
            if (result.get() == remove) {
                selectedProduct.deleteAssociatedPart(partToRemove);
    ...
    /**
     This method listens for the Save button on the Modify Product screen being pressed. 
     The method first validates user input with the validateUserInputProducts() method. With user input validated, a new Product object is generated. 
     The old, now outdated Product is removed from the total list of Products, and then the newly modified Product is added to the list to replace it.
    ...
    public void modifyProductSaveButtonPushed(ActionEvent event) throws IOException {
        ...
        //create new Product, with same ID (not a new one)
        Product modifiedProduct = new Product(Integer.parseInt(productIdTextField.getText()),
    ...
    /**
     This method is responsible for preemptively initializing the Modify Product form fields. 
     When a Product on the main screen is selected and the Modify button is pressed, this method is called to load the Product object's information into the Modify Product TextFields.
     */
    public void initializeFields() {
        ...
        //initialize the selectedProduct associatedPartList
